 ...
 * polled, never fetched for every skill up front). `lastUpgradedAt` (from
 * `recentlyUpgradedSkills`, already fetched with the list — no extra call)
 * drives the UPGRADED badge; the per-skill revision COUNT only exists once
 * this row's own on-demand fetch actually runs — `version_id` is a plain
 * UUID (confirmed against `skill_versioning.py`), so a revision count is
 * the one honest stand-in for "current version" this data supports. */
function SkillRow({
  skillId,
  name,
  autoGenerated,
  ...
  return (
    <div className="card-detail-overlay__group">
      ...
      >
        ...
        {lastUpgradedAt && <span className="card-detail-overlay__badge card-detail-overlay__badge--ok">UPGRADED</span>}
        {lastUpgradedAt && <span className="card-detail-overlay__row-meta">Last updated: {formatWhen(lastUpgradedAt)}</span>}
...
/** `SkillVersioning.list_recent_upgrades` is a fixed top-20 list across
 * EVERY skill (see its own docstring), never time-bounded — an entry in it
 * doesn't by itself mean "genuinely recently" upgraded (with few real
 * upgrades ever recorded, month-old ones can still make the top 20). This
 * is the one place that applies `RECENT_LEARNED_WINDOW_MS` (the same
 * "recently" `HudLeftPanel`'s own Skills card already uses) to build the
 * real subset, keyed by skill_id -> its own most recent qualifying
 * upgrade's `created_at`. */
function recentlyUpgradedSkills(recentUpgrades: SkillVersion[]): Map<string, string> {
  const cutoff = Date.now() - RECENT_LEARNED_WINDOW_MS
  const map = new Map<string, string>()
  for (const version of recentUpgrades) {
    if (new Date(version.created_at).getTime() < cutoff) continue
    const existing = map.get(version.skill_id)
    if (!existing || version.created_at > existing) map.set(version.skill_id, version.created_at)
  }
  return map
}

/** Genuinely-recently-upgraded skills first (newest upgrade first among
 * them), then everything else alphabetically — matches this overlay's own
 * "Sort recently upgraded skills first, then alphabetical" contract. */
function sortSkillsForDisplay(skills: Skill[], recentlyUpgraded: Map<string, string>): Skill[] {
  return [...skills].sort((a, b) => {
    const aUpgraded = recentlyUpgraded.get(a.id)
    const bUpgraded = recentlyUpgraded.get(b.id)
    if (aUpgraded && bUpgraded) return bUpgraded.localeCompare(aUpgraded)
    if (aUpgraded) return -1
    if (bUpgraded) return 1
...
function SkillsDetail({ skills }: { skills: ReturnType<typeof useSkills> }) {
  ...
  const recentlyUpgraded = recentlyUpgradedSkills(recentUpgrades)

  const query = search.trim().toLowerCase()
  const filtered = query ? list.filter((skill) => skill.name.toLowerCase().includes(query) || skill.domain.toLowerCase().includes(query)) : list
      ...
      {list.length === 0 ? (
        ...
        <>
          {list.length > 20 && (
            <input
              type="text"
              className="card-detail-overlay__search"
              placeholder="Search skills or domains…"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              aria-label="Search skills or domains"
            />
          )}
          {query && domains.length === 0 ? (
            <EmptyRow>{`No skills match "${search.trim()}".`}</EmptyRow>
          ) : (
            <div className="card-detail-overlay__list">
              {domains.map((domain) => {
                ...
                return (
                  <div key={domain} className="card-detail-overlay__group">
                    <h3 className="card-detail-overlay__section-title">
                      {domain} <span className="card-detail-overlay__row-meta">({domainSkills.length})</span>
                    </h3>
                    {sortSkillsForDisplay(domainSkills, recentlyUpgraded).map((skill) => (
                      <SkillRow
                        key={skill.id}
                        skillId={skill.id}
                        name={skill.name}
